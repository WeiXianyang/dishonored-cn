...
ap.add_argument('--src', default=(
    r'C:\SteamLibrary\steamapps\common\Dishonored-备份\Sub_Import\texts.db'))
args = ap.parse_args()
SRC = args.src
if not os.path.exists(SRC):
    print(f'[FAIL] texts.db 不存在: {SRC}')
    sys.exit(2)

orig = open(SRC, 'rb').read()
print(f'原文件: {len(orig)} bytes, md5={hashlib.md5(orig).hexdigest()}')

with tempfile.TemporaryDirectory(prefix='dh_roundtrip_') as temp:
    # 0 修改重建
    apply_textsdb(SRC, {}, temp)
    rebuilt = os.path.join(temp, 'Sub_Import', 'texts.db')
    out = open(rebuilt, 'rb').read()
